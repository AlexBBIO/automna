...
export function toAutomnaTokens(costMicrodollars) {
...
export async function logUsageEvent(input) {
    ...
    console.log(`[FLY-PROXY][Usage] user=${input.userId} type=${input.eventType} ` +
...
export function logUsageEventBackground(input) {
...
export function updateLastActiveBackground(machineId) {
    const now = Date.now();
...
export async function getUsedAutomnaTokens(userId) {
    const monthStart = new Date();
    monthStart.setUTCDate(1);
    monthStart.setUTCHours(0, 0, 0, 0);
    const monthStartUnix = Math.floor(monthStart.getTime() / 1000);
    const result = await db
        .select({ total: sql `COALESCE(SUM(automna_tokens), 0)`.as('total') })
        .from(usageEvents)
