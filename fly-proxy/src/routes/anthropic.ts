...
app.post("/messages", async (c) => {
  ...
  if (!token) return c.json({ type: "error", error: { type: "authentication_error", message: "Missing token" } }, 401);
  ...
  if (beta) forwardHeaders.set("anthropic-beta", beta);
  ...
  if (isStreaming) {
    ...
    let requestId: string | undefined, errorMessage: string | undefined;
    let sseBuffer = "";
    ...
    const stream = new ReadableStream({
      async start(streamController) {
        ...
        const keepaliveTimer = setInterval(() => {
          if (!realDataStarted) {
            try { keepaliveCount++; streamController.enqueue(keepaliveComment); } catch { clearInterval(keepaliveTimer); }
        ...
        try {
          ...
          while (true) {
            ...
            if (done) break;
            chunkCount++; totalBytes += value.length;
            ...
            try {
              ...
              for (const line of lines) {
                ...
                try {
                  ...
                  if (event.type === "error") errorMessage = JSON.stringify(event.error);
                } catch {}
              }
            } catch {}
          }
  ...
  try {
    ...
    logUsageBackground({ userId: auth.userId, provider: "anthropic", model, endpoint: "chat", inputTokens, outputTokens, cacheCreationTokens, cacheReadTokens, requestId: data.id as string, durationMs, error: data.type === "error" ? JSON.stringify(data.error) : undefined });
    const costMicro = calculateCostMicrodollars(model, inputTokens, outputTokens, cacheCreationTokens, cacheReadTokens);
