...
function PricingCard({ plan, isAnnual, isSignedIn, loading, onCheckout }: {
  ...
  return (
    ...
    >
      {plan.popular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-purple-600 rounded-full text-xs font-medium whitespace-nowrap">
          Most Popular
        </div>
      )}
      ...
      <p className="text-purple-300 text-xs md:text-sm font-medium mb-4 md:mb-6">
        {plan.tagline}
      </p>
      ...
      <ul className="space-y-2 md:space-y-3 mb-6 md:mb-8 flex-grow">
        ...
        {plan.footnote && (
          <li className="flex items-start gap-2 md:gap-3">
            ...
            <span className="text-gray-500 text-xs md:text-sm italic">{plan.footnote}</span>
          </li>
        )}
      </ul>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-950 to-black text-white">
      ...
      <nav className="container mx-auto px-4 md:px-6 py-4 md:py-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4 md:gap-6 text-gray-400 text-sm">
      ...
      <main className="py-8 md:py-16">
        ...
        <div className="max-w-3xl mx-auto mb-8 md:mb-12 px-4 md:px-6">
          <div className="bg-white/5 border border-white/10 rounded-xl md:rounded-2xl p-4 md:p-8">
            <h3 className="text-base md:text-xl font-bold mb-3 md:mb-4 text-center">What every plan includes</h3>
            <div className="grid grid-cols-4 gap-2 md:gap-4 text-center">
              <div className="p-1 md:p-3">
                <div className="text-xl md:text-2xl mb-1">🧠</div>
                <div className="text-gray-300 font-medium text-xs md:text-sm">Claude Opus</div>
                <div className="text-gray-500 text-xs hidden md:block">Latest model</div>
              </div>
              <div className="p-1 md:p-3">
                <div className="text-xl md:text-2xl mb-1">📞</div>
                <div className="text-gray-300 font-medium text-xs md:text-sm">Phone</div>
                <div className="text-gray-500 text-xs hidden md:block">Calls in & out</div>
              </div>
              <div className="p-1 md:p-3">
                <div className="text-xl md:text-2xl mb-1">📧</div>
                <div className="text-gray-300 font-medium text-xs md:text-sm">Email</div>
                <div className="text-gray-500 text-xs hidden md:block">Send & receive</div>
              </div>
              <div className="p-1 md:p-3">
                <div className="text-xl md:text-2xl mb-1">🌐</div>
                <div className="text-gray-300 font-medium text-xs md:text-sm">Browser</div>
                <div className="text-gray-500 text-xs hidden md:block">Research & browse</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center items-center gap-3 mb-8 md:mb-12 px-4">
          ...
          <span className={`text-sm ${isAnnual ? 'text-white font-medium' : 'text-gray-500'}`}>
            Annual
            <span className="ml-1 text-green-400 text-xs font-medium">Save 20%</span>
          </span>
        </div>
          ...
          {plans.map((plan, i) => (
            <div
              ...
              className={`w-2 h-2 rounded-full ${
                plan.popular ? 'bg-purple-500' : 'bg-white/20'
              }`}
            />
          ))}
        </div>
        ...
        <div className="mt-10 md:mt-16 max-w-3xl mx-auto text-center px-4 md:px-6">
          <p className="text-gray-500 text-xs md:text-sm">
            Not sure yet? <span className="text-purple-400">Lite at ${isAnnual ? '16' : '20'}/mo</span> gives you the full experience.
